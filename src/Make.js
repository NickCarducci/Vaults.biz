...
class Make extends React.PureComponent {
  ...
  componentDidMount = async () => {
    this.checkRecipientsOrEntity();
    this.openNew();
  };
  openNew = () => {
    const eventDate = this.props.pathname.split("/new/")[1];
    console.log(eventDate.split("/")[0]);
    if (eventDate.split("/")[0]) {
      const time = eventDate.split("/")[1];
      console.log(time.split(":")[0]);
      if (time.split(":"))
        this.setState({
          eventDate: new Date(
            new Date(eventDate.split("/")[0]).getTime() +
  ...
  checkRecipientsOrEntity = async () => {
    ...
    if (
      ...
      var recipients = this.props.location.state.recipients
        ? this.props.location.state.recipients
        : [];
      await Promise.all(
        ...
        this.setState({
          recipientSuggestionsProfiled,
          recipients,
          title: specialFormatting(this.props.location.state.sendTitlePlan),
    ...
    this.props.auth !== undefined &&
      addDoc(collection(firestore, "chats"), {
        ...
        threadId:
          ...
          this.state.entityId +
          (this.state.entityId ? "" : recipients.sort()),
        recipients: rec,
  ...
  async handleSave(note, method) {
    ...
    return foo && this.props.getNotes();
  }
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        {this.state.recipientSuggestionsProfiled.map((x) => x.username)}
        <textarea
          type="search"
          name="body"
          id="body"
